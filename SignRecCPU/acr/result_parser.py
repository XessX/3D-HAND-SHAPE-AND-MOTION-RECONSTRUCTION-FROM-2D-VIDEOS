    ...
    def parse_maps(self,outputs, meta_data, cfg):
        ...
        if 'params_pred' not in outputs and 'l_params_maps' in outputs and 'r_params_maps' in outputs:
            if len(l_batch_ids) != 0:
                ...
                l_batch_ids = torch.tensor([0])
                l_flat_inds = torch.tensor([0])
            ...
            if len(r_batch_ids) != 0:
                ...
                detection_flag.append(False)
                r_batch_ids = torch.tensor([0])
                r_flat_inds = torch.tensor([0])
        ...
        # again
        outputs['detection_flag'] = torch.Tensor(detection_flag)
        ...
        outputs['left_hand_num'] = torch.tensor([len(outputs['l_params_pred'])])
        outputs['right_hand_num'] = torch.tensor([len(outputs['r_params_pred'])])
