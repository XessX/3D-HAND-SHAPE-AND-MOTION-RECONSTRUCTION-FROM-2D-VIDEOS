...
videoPath=r'C:\SignRec f\Code\Data\train_rgb_front_clips\raw_videos'
outputPath=r'C:\SignRec f\Code\Data\train_rgb_front_clips\mano_videos'
dirList=listdir(videoPath)
for filename in dirList:
    try:    
        if filename.endswith(".mp4"):
            inFile=videoPath+'\\'+filename
            outFile=outputPath+'\\'+filename
            print(inFile)
            process_video_and_save_3d_mesh(inFile, outFile)            
            print('File Saved : ',outFile)            
    except:
        print('File Not Process : ',filename)
